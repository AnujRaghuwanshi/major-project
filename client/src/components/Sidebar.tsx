...
const Sidebar: React.FC<SidebarProps> = ({ username, role, id }) => {
  ...
  return (
    <div>
      ...
      <button
        className="md:hidden fixed top-4 right-4 z-50 p-2 text-blue-600"
      ...
      <div className="sticky left-0 top-0 flex h-screen w-fit flex-col justify-between border-r border-gray-200 bg-white pt-8 text-white max-md:hidden sm:p-4 xl:p-4 2xl:w-[355px]">
        ...
        <div className="profile-section flex flex-col gap-4 px-2 mt-4">
          <Image src="/logo.png" alt="Logo" width={150} height={60} className="ml-2" />
          <input
            id="search"
            name="search"
            placeholder="Search..."
        ...
        {/* Mobile Overlay */}
        <div
          className={`fixed inset-0 z-50 h-screen w-full bg-black bg-opacity-50 ${
            isOpen ? 'block' : 'hidden'
          } md:hidden`}
          onClick={toggleSidebar}
        ></div>
